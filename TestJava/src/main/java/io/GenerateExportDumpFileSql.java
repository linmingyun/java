...
public class GenerateExportDumpFileSql {
    ...
    public static void main(String[] args) {
        ...
        splitJoint(fileName);
        generateSql(splitJoint(entityCode),splitJoint(netCode),splitJoint(sstCode));
    }

    public static void generateSql(String entityCode, String netCode, String sstCode) {
        String custSqlStart = "expdp sbpopt/**** tables=TT_VW_POTENTIAL_CUSTOMER query=\\\"where ENTITY_CODE in \\(";
        String custSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile= pcustomer.dmp logfile=pcustomer.log  directory=DP";
        String intentSqlStart = "expdp sbpopt/#### tables=TT_VW_CUSTOMER_INTENT query=TT_VW_CUSTOMER_INTENT:\\\"where exists \\(select 1 from TT_VW_POTENTIAL_CUSTOMER t where t.ctm_id\\=ku$.ctm_id and t.entity_code\\=ku$.entity_code and t.ENTITY_CODE in \\(";
        String intentSqlEnd = "\\) and t.create_date \\> trunc\\(sysdate \\- 180\\)\\)\\\" dumpfile=follow.dmp logfile=follow.log  directory=DP";
        String followSqlStart = "expdp sbpopt/#### tables=TT_VW_FOLLOW_UP query=TT_VW_FOLLOW_UP:\\\"where exists \\(select 1 from TT_VW_POTENTIAL_CUSTOMER t where t.ctm_id\\=ku$.ctm_id and t.entity_code\\=ku$.entity_code and t.ENTITY_CODE in \\(";
        String followSqlEnd = "\\) and t.create_date \\> trunc\\(sysdate \\- 180\\)\\)\\\" dumpfile=intent.dmp logfile=intent.log  directory=DP";
        String succSqlStart = "expdp sbpopt/#### tables=TT_VW_SUCCESS query=TT_VW_SUCCESS:\\\"where exists \\(select 1 from TT_VW_POTENTIAL_CUSTOMER t where t.ctm_id\\=ku$.ctm_id and t.entity_code\\=ku$.entity_code and t.ENTITY_CODE in \\(";
        String succSqlEnd = "\\) and t.create_date \\> trunc\\(sysdate \\- 180\\)\\)\\\" dumpfile=success.dmp logfile= success.log  directory=DP";
        String loseSqlStart = "expdp sbpopt/**** tables=TT_VW_LOSE query=\\\"where ENTITY_CODE in \\(";
        String loseSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=lose.dmp logfile= lose.log  directory=DP";
        String showroomSqlStart = "expdp sbpopt/**** tables=tt_vw_showroom_reception query=\\\"where ENTITY_CODE in \\(";
        String showroomSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=showroom.dmp logfile= showroom.log  directory=DP";
        String assignSqlStart = "expdp sbpopt/**** tables=TT_OEM_CUSTOMER_ASSIGN query=\\\"where ENTITY_CODE in \\(";
        String assignSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=assign.dmp logfile= assign.log  directory=DP";
        String sanbaoSqlStart = "expdp sbpopt/**** tables=TT_SANBAO_VEHICLE query=\\\"where net_code in \\(";
        String sanbaoSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=vehicle.dmp logfile= vehicle.log  directory=DP";
        ...
        String tivwoemSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=oemcustomer.dmp logfile= oemcustomer.log  directory=DP";
    ...
    public static void generateSqlLastYear(String entityCode, String netCode, String sstCode) {
        ...
        String intentSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=follow.dmp logfile=follow.log  directory=DP";
        String followSqlStart = "expdp sbpopt/**** tables=TT_VW_FOLLOW_UP query=\\\"where ENTITY_CODE in \\(";
        String followSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=intent.dmp logfile=intent.log  directory=DP";
        String succSqlStart = "expdp sbpopt/**** tables=TT_VW_SUCCESS query=\\\"where ENTITY_CODE in \\(";
        String succSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=success.dmp logfile= success.log  directory=DP";
        String loseSqlStart = "expdp sbpopt/**** tables=TT_VW_LOSE query=\\\"where ENTITY_CODE in \\(";
        String loseSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=lose.dmp logfile= lose.log  directory=DP";
        String showroomSqlStart = "expdp sbpopt/**** tables=tt_vw_showroom_reception query=\\\"where ENTITY_CODE in \\(";
        String showroomSqlEnd = "\\) and create_date \\> trunc\\(sysdate \\- 180\\)\\\" dumpfile=showroom.dmp logfile= showroom.log  directory=DP";
